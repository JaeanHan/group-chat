...
`OutboxEvent`는 이벤트 전달 보장 정책으로 별도 문서에서 다룬다.
...
`ChatRoomMembership`은 현재 참여 상태를 나타내고, `ChatPeriod`는 메시지 조회 범위를 판단한다.
...
read cursor는 현재 `ChatPeriod` 범위 안에서만 갱신할 수 있다.
...
message.roomSequence > max(
  ...
  currentChatPeriod.joinedSequence
)
```

현재 열린 `ChatPeriod`가 없는 사용자는 unread count를 계산하지 않는다.
...
`ChatPeriod`는 메시지 조회 가능 범위를 나타내고, `ChatReadSession`은 read cursor 자동 갱신 여부를 판단한다.
...
서버는 `markRead` 요청을 현재 `ChatPeriod` 범위 안에서만 반영한다.
...
읽음 관련 WebSocket 이벤트는 Outbox 기본 대상이 아니다.
...
메시지 조회 가능 범위는 `ChatRoomMembership`만으로 판단하지 않고 현재 열린 `ChatPeriod` 기준으로 판단한다.
...
| 조건 | 읽음 처리 |
|---|---:|
| room ACTIVE + membership ACTIVE + 열린 ChatPeriod 있음 | 가능 |
| room CLOSED | 불가 |
| membership NONE | 불가 |
| membership LEFT | 불가 |
| 현재 열린 ChatPeriod 없음 | 불가 |
...
- ChatReadCursor.lastReadSequence는 현재 ChatPeriod 범위 안에서만 갱신된다.
...
읽음 관련 이벤트는 direct WebSocket 이벤트로 전파할 수 있으며, Outbox 기본 대상은 아니다.

---

## 16. 최종 상태전이 요약

### 16.1 ChatRoom.status
...
### 16.5 ChatReadCursor.lastReadSequence
...
### 16.6 ChatReadSession.runtimeStatus
...
### 16.8 WebSocketSession.runtimeStatus
...
14. 읽음 관련 WebSocket 이벤트는 Outbox 기본 대상이 아니다.
...
27. message 저장과 `outbox_event` 저장은 같은 DB transaction 안에서 처리한다.
28. WebSocket broadcast는 실시간 전달 경로이며 DB 상태를 대체하지 않는다.
29. WebSocket 연결/구독 상태는 DB 영속 상태가 아니라 서버 런타임 상태다.
30. WebSocket 상태는 `connectionId` 기준으로 관리한다.
31. WebSocket 재연결 후 클라이언트는 `afterSequence` 기반으로 누락 메시지를 복구한다.
32. join/leave/delete는 현재 DB 상태를 기준으로 중복 요청을 안전하게 처리한다.
33. sendMessage는 `clientMessageId`로 중복 메시지 생성을 방지한다.
34. editMessage는 `editVersion`으로 동시 수정 충돌을 방지한다.
35. 상태전이 실패는 401, 403, 404, 409 기준으로 구분한다.
