...
공용 오픈 채팅방 모듈은 다음 도메인 상태 또는 런타임 상태의 변화 규칙을 정의한다.
...
ChatParticipationPeriod.lifecycle
...
ChatReadCursor.lastReadSequence
  - 특정 userId가 특정 roomId에서 어디까지 읽었는지 나타내는 영속 상태

WebSocketSession.runtimeStatus
  - 서버 런타임에서 관리되는 연결 및 구독 상태

ChatReadSession.runtimeStatus
  - 특정 connection 또는 userId가 현재 room 화면을 보고 있는지 나타내는 런타임 상태
```
...
성공 시:
  - room.status = CLOSED
  - closedAt 기록
  - room에 열려 있는 ChatParticipationPeriod 종료
...
## 5. ChatRoomMembership.status 상태전이도
...
## 6. ChatParticipationPeriod

### 6.1 성격

`ChatParticipationPeriod`는 사용자가 room에 입장해 있던 메시지 조회 가능 구간이다.

`ChatRoomMembership`은 현재 참여 상태를 나타내고, `ChatParticipationPeriod`는 메시지 조회 범위를 판단한다.

사용자는 현재 `ChatParticipationPeriod`가 시작된 이후 생성된 메시지만 조회할 수 있다.

재입장 시 새로운 `ChatParticipationPeriod`가 생성되며, 이전 `ChatParticipationPeriod` 또는 퇴장 중 생성된 메시지는 현재 `ChatParticipationPeriod` 조회 범위에 포함되지 않는다.

---

### 6.2 joinRoom과 ChatParticipationPeriod

`joinRoom` 성공 시 새로운 `ChatParticipationPeriod`가 생성된다.
...
### 6.3 leaveRoom과 ChatParticipationPeriod

`leaveRoom` 성공 시 현재 열려 있는 `ChatParticipationPeriod`가 닫힌다.

```text
leftSequence = leaveRoom 처리 시점의 room.lastMessageSequence
```

`closeRoom` 성공 시 해당 room에 열려 있는 모든 `ChatParticipationPeriod`가 닫힌다.
...
일반 USER의 메시지 조회는 현재 `ChatParticipationPeriod` 범위 안에서만 가능하다.
...
현재 열린 `ChatParticipationPeriod`가 없는 사용자는 해당 room의 메시지를 조회할 수 없다.
...
`ChatReadCursor`는 사용자가 특정 room에서 어디까지 읽었는지 나타내는 영속 상태다.

읽음 상태의 source of truth는 `ChatReadCursor`다.

실시간 read receipt 표시는 WebSocket 이벤트로 보정할 수 있지만, 최종 정확한 값은 메시지 조회 시 `ChatReadCursor` 기준으로 다시 계산한다.
...
[NOT_CREATED]
  -- joinRoom -->
lastReadSequence = joinedSequence
...
`ChatReadCursor`는 roomSequence 기준으로만 갱신한다.

`ChatReadCursor.lastReadSequence`는 감소하지 않는다.

```text
현재 lastReadSequence = 100
요청 readSequence = 90
  -> 상태 변화 없음
```

read cursor는 현재 `ChatParticipationPeriod` 범위 안에서만 갱신할 수 있다.
...
`ChatReadSession`은 사용자가 현재 room 화면을 보고 있는지 나타내는 런타임 상태다.

`ChatReadSession`은 DB 영속 상태가 아니며, WebSocket connection 또는 application foreground 상태에 따라 서버 런타임에서 관리한다.

`ChatParticipationPeriod`는 메시지 조회 가능 범위를 나타내고, `ChatReadSession`은 read cursor 자동 갱신 여부를 판단한다.
...
클라이언트는 room enter, room leave, foreground/background, visibility change 시 현재 room view 상태를 서버에 알릴 수 있다.

서버는 `VISIBLE` 상태인 active viewer에게 새 메시지를 broadcast한 경우 해당 사용자의 `ChatReadCursor`를 새 `roomSequence`까지 갱신할 수 있다.
...
클라이언트는 reconnect, foreground, room enter, scroll-to-bottom 시 `markRead(latestVisibleSequence)`를 보낼 수 있다.

서버는 `markRead` 요청을 현재 `ChatParticipationPeriod` 범위 안에서만 반영한다.

`markRead`는 read cursor를 감소시키지 않는다.
...
membership.status == ACTIVE
현재 열린 ChatParticipationPeriod 존재
```
...
### 10.6 WebSocket 인증 및 재연결 원칙

WebSocket 연결 시 JWT를 검증한다.

구독 요청 시마다 `room.status`, `room.operationStatus`, `membership.status`, `ChatParticipationPeriod` 유효성을 다시 검증한다.
...
메시지 조회 가능 범위는 `ChatRoomMembership`만으로 판단하지 않고 현재 열린 `ChatParticipationPeriod` 기준으로 판단한다.
...
### 12.4 ChatReadCursor 기준

| 조건 | 읽음 처리 |
|---|---:|
| room ACTIVE + membership ACTIVE + 열린 ChatParticipationPeriod 있음 | 가능 |
| room CLOSED | 불가 |
| membership NONE | 불가 |
| membership LEFT | 불가 |
| 현재 열린 ChatParticipationPeriod 없음 | 불가 |
...
읽음 관련 이벤트는 direct WebSocket 이벤트로 전파할 수 있으며, 영속 전달 보장 대상은 아니다.
...
### 15.5 ChatReadCursor.lastReadSequence
...
### 15.6 ChatReadSession.runtimeStatus
...
VISIBLE
  -- room hidden or disconnect -->
HIDDEN
```

---

### 15.7 ChatMessage.visibility
...
### 15.8 WebSocketSession.runtimeStatus
...
7. `ChatRoomMembership.status`는 `ACTIVE`, `LEFT`를 사용하고, 참여 이력이 없으면 `NONE` 개념 상태로 본다.
8. 메시지 조회 가능 범위는 `ChatParticipationPeriod` 기준으로 판단한다.
9. 사용자는 현재 `ChatParticipationPeriod` 시작 이후 생성된 메시지만 조회할 수 있다.
10. `ChatReadCursor`는 userId가 room에서 어디까지 읽었는지 나타낸다.
11. unread count와 read receipt count는 `ChatReadCursor` 기준으로 계산한다.
12. read receipt count는 작성자 본인을 제외한다.
13. active-visible viewer는 새 메시지 broadcast 시 read cursor를 자동 갱신할 수 있다.
14. 읽음 관련 WebSocket 이벤트는 영속 전달 보장 대상이 아니다.
15. 메시지 수정은 상태 전이가 아니라 속성 변경이다.
16. 메시지 삭제는 `MessageVisibility` 전이로 관리한다.
17. 삭제된 메시지는 물리 삭제하지 않고 `DELETED_BY_USER`로 표시한다.
18. 일반 USER 응답에서는 삭제된 메시지의 content를 노출하지 않는다.
19. `roomSequence`는 room 단위 도메인 sequence이며 DB auto increment로 대체하지 않는다.
20. `roomSequence`는 DB transaction에서 room 단위로 부여한다.
21. 서비스 서버 메모리에서 `roomSequence`를 증가시키지 않는다.
22. `roomSequence`는 저장에 성공한 메시지 기준으로 연속 증가한다.
23. 메시지 전송 요청은 프론트엔드가 생성한 `clientMessageId`를 포함한다.
24. 서버는 `roomId + senderUserId + clientMessageId`를 unique로 관리한다.
25. 방 생성 요청은 프론트엔드가 생성한 `clientRequestId`를 포함할 수 있다.
26. 서버는 `ownerUserId + clientRequestId`를 unique로 관리할 수 있다.
27. WebSocket broadcast는 실시간 전달 경로이며 DB 상태를 대체하지 않는다.
28. WebSocket 연결/구독 상태는 DB 영속 상태가 아니라 서버 런타임 상태다.
29. WebSocket 상태는 `connectionId` 기준으로 관리한다.
30. WebSocket 재연결 후 클라이언트는 `afterSequence` 기반으로 누락 메시지를 복구한다.
31. join/leave/delete는 현재 DB 상태를 기준으로 중복 요청을 안전하게 처리한다.
32. sendMessage는 `clientMessageId`로 중복 메시지 생성을 방지한다.
33. editMessage는 `editVersion`으로 동시 수정 충돌을 방지한다.
34. 상태전이 실패는 401, 403, 404, 409 기준으로 구분한다.
