...
`firstUnreadSequence`는 채팅 화면 최초 진입 시 첫 번째 안 읽은 메시지 위치를 알려주는 표시 기준이다.

`firstUnreadSequence`는 별도 저장 값이 아니라 현재 사용자의 `ChatReadCursor.lastReadSequence`와 현재 열린 `ChatParticipationPeriod.joinedSequence`를 기준으로 계산한다.
...
firstUnreadSequence:
  baseSequence 이후 첫 번째 message.roomSequence
```

baseSequence 이후 메시지가 없으면 `firstUnreadSequence`는 null이다.

`beforeSequence` 또는 `afterSequence`를 지정한 페이지네이션 조회와 `catchUpMessages` 응답에서는 `firstUnreadSequence`를 사용하지 않으며 null로 둘 수 있다.
...
- beforeSequence와 afterSequence가 모두 없으면 채팅 화면 최초 진입 조회로 본다.
- 최초 진입 조회에서 firstUnreadSequence가 있으면 해당 sequence를 포함해 이후 메시지를 limit까지 조회한다.
- 최초 진입 조회에서 firstUnreadSequence가 없으면 현재 참여 구간에서 가장 최근 메시지를 조회한다.
