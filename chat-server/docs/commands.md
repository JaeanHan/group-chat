...
lastReadSequence로 ChatReadCursor가 실제 증가한 경우 READ_CURSOR_UPDATED를 생성할 수 있다.
...
READ_CURSOR_UPDATED는 readerKey 기준 read cursor 변경을 room 구독자에게 전파하는 데 사용한다.

서버는 메시지별 read receipt count 갱신 이벤트를 별도로 생성하지 않는다.

클라이언트는 READ_CURSOR_UPDATED와 ReadCursorSnapshot을 기준으로 메시지별 readReceiptCount를 다시 계산한다.

이 이벤트는 실시간 표시 보조 이벤트이며, 영속 전달 보장 대상은 아니다.
