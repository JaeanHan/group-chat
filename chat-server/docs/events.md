...
- senderReaderKey는 read receipt count 계산에서 작성자 본인을 제외하기 위해 사용한다.
...
`READ_CURSOR_UPDATED`는 특정 reader의 read cursor가 갱신되었음을 전파한다.

기본 수신 대상은 해당 room을 구독 중인 connection이다.
...
```text
- readerKey가 로컬 ReadCursorSnapshot에 있으면 lastReadSequence를 갱신한다.
- lastReadSequence가 로컬 snapshot의 해당 readerKey 값보다 작거나 같으면 중복 또는 오래된 이벤트로 보고 무시한다.
- readerKey가 currentUserReaderKey와 같으면 현재 사용자의 read cursor가 다른 connection에서 갱신된 것으로 볼 수 있다.
- readerKey가 로컬 snapshot에 없으면 snapshot이 stale한 것으로 보고 getReadCursorSnapshot으로 교체할 수 있다.
- snapshot 갱신 후 현재 표시 중인 메시지의 read receipt count를 다시 계산한다.
```
...
읽음 표시와 read cursor snapshot은 `getReadCursorSnapshot` 조회 결과로 보정한다.

room 목록의 unread count는 `getUnreadCounts` 또는 목록 재조회로 보정한다.
