...
읽음:
  서버가 계산한 room unread count와 reader cursor snapshot을 보정 기준으로 사용한다.
...
실시간 연결:
  catchUpMessages 결과는 ClientRoomSubscription의 catchUpRequired, lastReceivedSequence 보정 입력으로 사용할 수 있다.
```
...
read cursor snapshot은 `READ_CURSOR_UPDATED` event로 빠르게 보정할 수 있지만, query 결과로 다시 보정할 수 있다.

room unread count는 server event로 직접 확정하지 않고 `getUnreadCounts` 또는 `listRooms` 결과로 보정한다.
...
방 목록 화면의 `lastMessagePreview`와 `unreadCount`는 polling, foreground 복귀, 수동 새로고침 시점의 query 결과로 보정한다.

`lastMessagePreview`는 `listRooms` 결과로 보정하고, `unreadCount`는 `getUnreadCounts` 또는 `listRooms` 결과로 보정한다.

server event가 유실되어도 메시지는 `catchUpMessages` 또는 `listMessages`로, room 상태는 `listRooms` 또는 `getRoomDetail`로, room unread count와 read cursor snapshot은 관련 read query로 보정할 수 있어야 한다.
...
방 목록 화면은 polling, foreground 복귀, 수동 새로고침 시점에 `getUnreadCounts`를 실행해 room unread count를 보정할 수 있다.
...
ReadCursorSnapshot
  ...
  -> ClientMessageReadReceipt
     현재 표시 중인 메시지의 senderReaderKey와 snapshot.readers를 기준으로 readReceiptCount와 derivedUnreadReceiptCount를 다시 계산한다.
```
