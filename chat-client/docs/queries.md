...
읽음:
  서버가 계산한 unread count, read receipt count를 보정 기준으로 사용한다.
...
읽음:
  ...
  getReadReceiptCounts는 현재 표시 중인 메시지 구간의 unread receipt count를 보정한다.
...
unread count와 read receipt count는 server event로 빠르게 반영할 수 있지만, query 결과로 다시 보정할 수 있다.
...
server event가 유실되어도 메시지는 `catchUpMessages` 또는 `listMessages`로, room 상태는 `listRooms` 또는 `getRoomDetail`로, unread count와 read receipt count는 관련 read query로 보정할 수 있어야 한다.
...
```text
사용자가 자신이 보낸 메시지를 보고 있음
READ_CURSOR_UPDATED event가 유실되었거나 중복 반영되었을 수 있음
WebSocket reconnect 이후 read receipt 표시를 보정해야 함
메시지 구간의 안 읽은 사람 수 표시를 보정해야 함
```
...
```text
직접 반영:
  ClientMessage.unreadReceiptCount
```

#### 반영 기준

`ReadReceiptSummary[]`는 메시지 구간의 unread receipt 표시에 직접 반영된다.
...
ReadReceiptSummary[]
  -> ClientMessage.unreadReceiptCount
     messageId가 같은 ClientMessage가 있으면 unreadReceiptCount를 메시지 옆 안 읽은 사람 수 표시값으로 반영한다.

  참고 값:
     readReceiptCount는 서버 계산 결과로 참고할 수 있다.
     화면에 안 읽은 사람 수를 표시하는 경우 unreadReceiptCount를 우선 사용한다.
```

작성자 본인은 unread receipt count의 표시 대상에 포함하지 않는다.

`getReadReceiptCounts`는 read cursor 또는 read session을 변경하지 않는다.

read receipt query는 현재 표시 중인 메시지 구간의 읽음 표시 count를 보정하는 조회다.

#### 인터랙션 영향

내가 보낸 메시지 옆의 안 읽은 사람 수를 보정한다.

읽음 표시 조회 실패는 메시지 전송 성공 여부나 메시지 표시 여부를 바꾸지 않는다.
