# 오픈 채팅방 클라이언트 도메인 관점

## 1. 목적

이 문서는 오픈 채팅방의 비즈니스 핵심 요소를 클라이언트가 어떤 화면 의미로 해석하는지 정의한다.

클라이언트는 서버의 source of truth를 소유하지 않는다.

클라이언트는 서버에서 확인한 정보, 사용자 입력, 진행 중인 요청을 조합해 사용자가 이해할 수 있는 화면을 만든다.

---

## 2. 도메인 경계

클라이언트 도메인 경계는 서버에서 받은 정보를 화면의 행동 가능 상태로 해석하는 지점에서 생긴다.

클라이언트는 서버에서 받은 정보를 그대로 노출하지 않고, 진행 중인 요청을 함께 고려해 현재 화면에서 무엇을 보여주고 어떤 interaction을 허용할지 결정한다.

이 문서는 비즈니스 핵심 요소를 다음 네 관점으로 나누어 설명한다.

```text
서버에서 확인한 정보:
  서버가 query 결과, command 응답, server event로 클라이언트에 알려준 정보.
  server event는 WebSocket 같은 실시간 연결을 통해 들어올 수 있다.
  화면 판단의 기준이 되지만, 단독으로 interaction 전체를 결정하지 않는다.

화면에서 판단한 결과:
  서버에서 확인한 정보와 진행 중인 요청을 조합해
  현재 화면에서 무엇을 보여주고 어떤 interaction을 허용할지 판단한 결과.

진행 중인 요청:
  사용자가 실행했지만 아직 서버가 성공/실패를 확정하지 않은 작업.
  화면에서는 pending, retry, disabled 같은 형태로 드러난다.

화면 동작과 표현:
  화면에서 판단한 결과가 사용자에게 드러나는 방식.
  이 문서에서는 구체적인 component 상태가 아니라,
  비즈니스 해석이 UI로 이어진다는 도착점으로만 다룬다.
```

이 문서는 비즈니스 핵심 요소별로 서버에서 확인한 정보와 화면에서 판단한 결과의 관계를 우선 다룬다.

각 비즈니스 요소 섹션에서는 서버에서 확인한 정보와 화면에서 판단한 결과를 중심으로 설명한다.

진행 중인 요청은 해당 요소의 의미를 바꾸는 경우에만 포함한다.

화면 동작과 표현은 각 요소의 최종 도착점으로 전제하되, 구체적인 표현 방식은 이 문서에서 정의하지 않는다.

---

## 3. 채팅방

채팅방은 사용자가 발견하고, 상세를 확인하고, 참여 여부를 결정하는 대상이다.

클라이언트에서 채팅방은 다음 질문을 판단하기 위한 기준이다.

```text
- 사용자는 어떤 room을 발견할 수 있어야 하는가
- 사용자는 어떤 room의 맥락을 이해할 수 있어야 하는가
- 사용자는 어떤 room에 참여 의사를 표현할 수 있어야 하는가
- 사용자는 어떤 room을 더 이상 사용할 수 없다고 이해해야 하는가
```

이 판단을 위해 클라이언트는 다음 정보를 사용한다.

```text
ClientRoomSummary:
  roomId
  name
  accessStatus
  operationStatus
  participantCount
  lastMessagePreview nullable
  unreadCount

ClientRoomDetail:
  roomId
  name
  accessStatus
  operationStatus
  owner
  participantCount
  currentUserParticipationStatus
```

화면에서는 다음 결과를 판단한다.

```text
방 목록 표시 여부:
  사용자가 room을 발견할 수 있는가

방 상세 사용 가능 여부:
  사용자가 room 정보를 보고 참여 여부를 판단할 수 있는가

채팅 화면 진입 가능 여부:
  현재 사용자가 room에서 메시지 조회/전송 흐름에 들어갈 수 있는가

메시지 입력 가능 여부:
  현재 사용자가 이 room에서 메시지를 보낼 수 있는가
```

방 목록에 보인다는 것은 채팅 화면을 사용할 수 있다는 뜻이 아니다.

방 상세를 볼 수 있다는 것도 현재 사용자가 참여 중이라는 뜻이 아니다.

채팅 화면 사용 가능 여부는 채팅방 정보와 현재 사용자의 참여 정보를 함께 기준으로 판단한다.

운영 개입 중인 room은 종료된 room과 다르게 해석한다.

운영 개입 중이어도 기존 참여자의 메시지 사용이 가능할 수 있다.

따라서 클라이언트는 운영 개입 상태를 곧바로 접근 불가로 해석하지 않는다.

---

## 4. 참여

참여는 현재 사용자가 특정 room에서 채팅 화면을 사용할 수 있는지 결정하는 기준이다.

클라이언트는 참여를 전체 참여자 목록이 아니라 현재 사용자와 특정 room 사이의 관계로 해석한다.

클라이언트에서 참여는 다음 질문을 판단하기 위한 기준이다.

```text
- 사용자는 자신이 room 안에 있는지 밖에 있는지 이해할 수 있어야 하는가
- 사용자는 참여 전, 참여 중, 나감 상태에 따라 다른 행동을 할 수 있어야 하는가
- 사용자는 재입장 후 이전 메시지를 이어 볼 수 없음을 이해해야 하는가
```

이 판단을 위해 클라이언트는 다음 정보를 사용한다.

```text
ClientParticipation:
  roomId
  status
  joinedAt optional
  leftAt optional
```

화면에서는 다음 결과를 판단한다.

```text
참여 가능 여부:
  room 상세에서 참여 버튼을 보여줄 수 있는가

채팅 화면 사용 가능 여부:
  메시지 조회, 메시지 전송, room 구독을 사용할 수 있는가

나간 room 처리:
  현재 채팅 화면을 닫고 이전 참여 구간 메시지를 현재 메시지로 이어 보지 않도록 할 것인가
```

참여 요청이 진행 중이면 화면에서 판단한 결과가 달라질 수 있다.

```text
joinRoom 요청 중:
  참여 버튼과 채팅 화면 진입을 pending으로 표현한다.

leaveRoom 요청 중:
  채팅 화면 이탈과 구독 정리를 pending으로 표현한다.
```

재입장 후 메시지 조회 범위는 서버가 확정한 현재 참여 구간을 따른다.

클라이언트는 모든 room에 대해 참여 전 상태를 미리 만들 필요는 없다.

참여 정보는 room 상세 조회, 참여 command 응답, server event를 통해 필요한 room에 대해서만 해석한다.

---

## 5. 메시지

메시지는 서버에서 확인한 메시지와 서버 확정 전 임시 메시지로 나뉜다.

클라이언트에서 메시지는 대화 맥락을 화면에 표시하고 사용자의 전송, 수정, 삭제 행동을 연결하는 단위다.

클라이언트에서 메시지는 다음 질문을 판단하기 위한 기준이다.

```text
- 사용자는 어떤 메시지가 서버에 확정되었는지 이해할 수 있어야 하는가
- 사용자는 아직 전송 중인 메시지를 구분할 수 있어야 하는가
- 사용자는 메시지의 수정/삭제 결과를 일관되게 볼 수 있어야 하는가
- 사용자는 삭제된 메시지의 내용을 일반 메시지처럼 보지 않아야 하는가
- 사용자는 자신이 보낸 메시지의 읽음 정도를 이해할 수 있어야 하는가
```

클라이언트는 메시지를 두 모델로 해석한다.

서버에서 확인한 메시지와 서버 확정 전 메시지는 식별자, 순서, 실패 처리 기준이 다르기 때문이다.

서버에서 확인한 메시지는 다음 정보로 다룬다.

```text
ConfirmedMessage:
  messageId
  roomId
  sender
  status
  type
  content nullable
  visibility
  roomSequence
  clientMessageId
  editVersion
  unreadReceiptCount optional
  sentAt
  editedAt nullable
  deletedAt nullable
```

메시지 전송 요청은 서버 확정 전에도 화면에 표시되어야 하므로 다음 정보로 다룬다.

```text
PendingMessage:
  clientMessageId
  roomId
  sender
  status
  content
  createdAt
```

화면에서는 다음 결과를 판단한다.

```text
표시할 메시지:
  ConfirmedMessage와 PendingMessage를 함께 고려한 채팅 화면 메시지

메시지 순서:
  서버가 확정한 room 단위 메시지 순서를 최종 기준으로 사용한다.

메시지 수정 가능 여부:
  작성자, 참여 상태, editVersion, 서버 command 결과를 함께 기준으로 판단한다.

메시지 삭제 가능 여부:
  작성자, 참여 상태, editVersion, 서버 command 결과를 함께 기준으로 판단한다.
```

클라이언트는 같은 메시지가 여러 경로로 도착해도 하나의 메시지로 병합해야 한다.

서버 확정 메시지가 도착하면 같은 `clientMessageId`를 가진 임시 메시지를 서버 확정 메시지로 대체한다.

수정/삭제 이벤트가 늦게 도착할 수 있으므로 클라이언트는 메시지 버전을 기준으로 오래된 변경을 무시해야 한다.

삭제된 메시지는 content를 일반 메시지처럼 노출하지 않는다.

삭제 메시지를 placeholder로 표시할지 목록에서 제외할지는 UI 정책에서 정한다.

---

## 6. 읽음과 표시

읽음은 사용자가 특정 room에서 어디까지 읽었는지를 나타내는 기준이다.

클라이언트에서 읽음은 다음 질문을 판단하기 위한 기준이다.

```text
- 사용자는 자신이 어디까지 읽었는지 기준을 가질 수 있어야 하는가
- 사용자는 room별 unread count를 이해할 수 있어야 하는가
- 사용자는 자신이 보낸 메시지를 몇 명이 아직 읽지 않았는지 볼 수 있어야 하는가
- 사용자는 현재 보고 있는 room의 읽음 상태가 자연스럽게 보정되어야 하는가
```

이 판단을 위해 클라이언트는 다음 정보를 사용한다.

```text
ClientReadState:
  roomId
  userId
  readCursor
  readSessionStatus
  unreadCount
```

화면에서는 다음 결과를 판단한다.

```text
read cursor:
  사용자가 특정 room에서 읽은 마지막 메시지 위치

unread count:
  room 단위로 아직 읽지 않은 메시지 수

unread receipt count:
  메시지 단위로 아직 읽지 않은 대상 참여자 수
```

읽음 보정을 위해 클라이언트가 보내는 신호는 다음과 같다.

```text
read session:
  사용자가 채팅 화면을 보고 있음을 서버에 알리는 런타임 정보

markRead:
  클라이언트가 적절한 시점에 보내는 read cursor 보정 요청
```

read cursor의 최종 값은 서버가 확정한다.

클라이언트는 새 메시지마다 읽음 갱신 요청을 보내지 않는다.

대신 room enter, foreground, scroll-to-bottom, room leave 같은 적절한 시점에 read cursor 보정을 요청한다.

읽음 관련 실시간 이벤트가 유실되어도 서버 조회 결과로 다시 계산할 수 있어야 한다.
