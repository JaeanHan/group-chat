...
메시지 목록, unread count, unread receipt count는 서버 조회 결과 또는 누락 복구 결과로 다시 보정될 수 있어야 한다.
...
삭제가 확정되면 메시지는 content를 노출하지 않는 삭제 표시 상태로 바뀐다.
...
클라이언트는 읽음 상태를 read cursor, read session, unread count, unread receipt count로 나누어 해석한다.
...
read cursor는 unread count와 read receipt count의 기준이 된다.
...
read cursor 보정이 확정되면 unread count와 unread receipt count 표시를 보정한다.

---

### 6.3 unread count 표시

unread count는 사용자가 아직 읽지 않은 메시지 수를 나타낸다.

클라이언트는 unread count를 방 목록 badge 또는 room 진입 전 표시 정보로 사용할 수 있다.

unread count는 read cursor 이후의 메시지를 기준으로 해석한다.

unread count는 실시간으로 먼저 바뀔 수 있지만, 최종 값은 서버 조회 결과로 보정한다.

---

### 6.4 unread receipt count 표시

클라이언트는 메시지 옆에 read receipt count 자체가 아니라 unread receipt count를 표시할 수 있다.

unread receipt count는 해당 메시지를 아직 읽지 않은 표시 대상 참여자 수다.
...
작성자 본인은 unread receipt count에도 포함되지 않는 것으로 해석한다.

unread receipt count는 실시간 이벤트로 먼저 바뀔 수 있지만, 최종 값은 서버 조회 결과로 보정한다.
